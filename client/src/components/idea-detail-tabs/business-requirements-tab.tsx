...
import { Loader2, RefreshCw, Copy, FileText, Flame, Hammer, AlertTriangle, RotateCcw } from "lucide-react";
...
export function BusinessRequirementsTab({ ideaId }: BusinessRequirementsTabProps) {
    const { toast } = useToast();
    const [businessRequirementsNotes, setBusinessRequirementsNotes] = useState('');

    const {
        document: businessRequirements,
        isLoading: isLoadingBusinessRequirements,
        isGenerating: businessRequirementsGenerating,
        isTimedOut: businessRequirementsTimedOut,
        generate: generateBusinessRequirements,
        deleteDoc: deleteBusinessRequirements,
        fetchDocument: fetchBusinessRequirements
    } = useDocument(ideaId, "BusinessRequirements");
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                {!businessRequirements && (
                    <Button
                        size="sm"
                        variant="outline"
                        onClick={() => generateBusinessRequirements(businessRequirementsNotes)}
                        disabled={businessRequirementsGenerating}
                    >
            ...
            {isLoadingBusinessRequirements ? (
                        ...
                        <div>
                            ...
                            <div className="mt-6 flex justify-between items-center">
                                ...
                                <Button
                                    ...
                                    variant="outline"
                                    onClick={async () => {
                                        if (businessRequirements?.id) {
                                            await deleteBusinessRequirements();
                                            generateBusinessRequirements(businessRequirementsNotes);
                                        }
                                    }}
                                    disabled={businessRequirementsGenerating}
                                >
                ...
                <div className="py-8">
                    ...
                    <div className="flex justify-center">
                        ...
                        >
                            <Flame className="mr-2 h-4 w-4" />
                            {businessRequirementsGenerating ? 'Generating...' : 'Generate Business Requirements'}
                        </Button>
                    </div>
                </div>
            )}
