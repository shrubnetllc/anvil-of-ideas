...
export function UltimateWebsiteTab({ ideaId }: UltimateWebsiteTabProps) {
    ...
    // Fetch the LeanCanvas to build business_description
    const { data: canvas } = useQuery({
        queryKey: [`/api/ideas/${ideaId}/canvas`],
        queryFn: async () => {
            const res = await fetch(`/api/ideas/${ideaId}/canvas`);
    ...
    const generateMutation = useMutation({
        mutationFn: async () => {
            const problem = canvas?.problem || "";
            const solution = canvas?.solution || "";
            const businessDescription = `The Problem: ${problem} The Solution: ${solution}`;
